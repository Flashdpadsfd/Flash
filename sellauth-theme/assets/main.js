/* FlashShp Theme — SellAuth Edition · main.js
 *
 * CONFIG : Set your Shop ID before this script loads:
 *   <script>window.NEXUS_SHOP_ID = 12345;</script>
 *
 * Each .product-card needs:
 *   data-product-id="<sellauth_product_id>"
 *   data-variant-id="<sellauth_variant_id>"   (optional, 0 if none)
 */
(function () {
  'use strict';
  ...
  document.querySelectorAll('.btn-buy').forEach(function (btn) {
    btn.addEventListener('click', function (e) {
      ...
      if (!productId) {
        console.warn('[FlashShp] Missing data-product-id on .product-card — replace placeholder IDs with your real SellAuth product IDs.');
        return;
      }
      ...
      if (variantId) opts.cart[0].variantId = variantId;
      window.sellAuthEmbed.checkout(btn, opts);
    });
  });
  ...
  function filterAndSort() {
    ...
    if (resultsCount) resultsCount.textContent = visible.length + ' produit' + (visible.length !== 1 ? 's' : '');
    ...
    if (!visible.length) {
      if (!empty) {
        ...
        empty.innerHTML = '<div class="empty-state__icon">🔍</div>'
          ...
          + '<div class="empty-state__sub">Essayez une autre recherche ou catégorie</div>';
  ...
  document.querySelectorAll('.sidebar__item[data-cat]').forEach(function (item) {
    item.addEventListener('click', function () {
      ...
      var ttl = $('pageTitle');
      if (ttl) ttl.textContent = item.dataset.label || 'Tous les produits';
      filterAndSort();
    });
  });
